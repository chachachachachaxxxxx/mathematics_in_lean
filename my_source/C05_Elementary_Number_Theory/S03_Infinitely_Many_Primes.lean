...
theorem primes_infinite_problem : ∀ n, ∃ p > n, Nat.Prime p := by
  intro n
...
theorem primes_infinite: ∀ n, ∃ p > n, Nat.Prime p := by
  ...
  have : p ∣ Nat.factorial (n + 1) := by
    ...
    linarith
  have : p ∣ 1 := by
    have eq : 1 = (n + 1).factorial + 1 - (n+1).factorial := by norm_num
    rw [eq]
    apply Nat.dvd_sub' pdvd this
  show False
  rw [Nat.dvd_one] at this
  have pneone : p ≠ 1 := by linarith [pp.two_le]
  exact pneone this

theorem primes_infinite_answer : ∀ n, ∃ p > n, Nat.Prime p := by
  intro n
  have : 2 ≤ Nat.factorial (n + 1) + 1 := by
...
theorem mem_of_dvd_prod_primes {s : Finset ℕ} {p : ℕ} (prime_p : p.Prime) :
  ...
  rw [mem_insert]
  rcases h₁ with pdvd | pns
  · left; apply Nat.Prime.eq_of_dvd_of_prime prime_p h₀.1 pdvd
  · right; apply ih h₀.2 pns

example (s : Finset ℕ) (x : ℕ) : x ∈ s.filter Nat.Prime ↔ x ∈ s ∧ x.Prime :=
  mem_filter

theorem primes_infinite'_problem : ∀ s : Finset Nat, ∃ p, Nat.Prime p ∧ p ∉ s := by
  intro s
  by_contra h
  push_neg  at h
...
theorem primes_infinite'_answer: ∀ s : Finset Nat, ∃ p, Nat.Prime p ∧ p ∉ s := by
  intro s
  by_contra h
  push_neg  at h
  set s' := s.filter Nat.Prime with s'_def
...
--has an element not in finite set
...
theorem ex_finset_of_bounded (Q : ℕ → Prop) [DecidablePred Q] :
  ...
  exact hn k
--some property could be use to filter
--or some property means infinity
...
theorem exists_prime_factor_mod_4_eq_3_problem {n : Nat} (h : n % 4 = 3) :
...
theorem exists_prime_factor_mod_4_eq_3 {n : Nat} (h : n % 4 = 3) :
  ...
  · rcases aux mdvdn mge2 mltn with ⟨nmdvdn, nmltn⟩
    ...
    · use (n / m)
    · rcases ih (n / m) nmltn h1 nmp with ⟨p, pp, pdvdm, pmod3⟩
      use p, pp
      constructor
      · apply Nat.dvd_trans pdvdm nmdvdn
      · use pmod3

theorem exists_prime_factor_mod_4_eq_3_answer {n : Nat} (h : n % 4 = 3) :
    ∃ p : Nat, p.Prime ∧ p ∣ n ∧ p % 4 = 3 := by
  by_cases np : n.Prime
  · use n
  induction' n using Nat.strong_induction_on with n ih
...
theorem primes_mod_4_eq_3_infinite : ∀ n, ∃ p > n, Nat.Prime p ∧ p % 4 = 3 := by
  ...
  have pne3 : p ≠ 3 := by
    intro peq3
    --3 | 4 * ∏ i ∈ s.erase 3, i is False
    rw [peq3] at pdvd
    simp at pdvd
    sorry
  have : p ∣ 4 * ∏ i in erase s 3, i := by
    --p is one member of s, but not 3
    sorry
  have : p ∣ 3 := by
    convert Nat.dvd_sub' pdvd this
    norm_num
  have : p = 3 := by
    apply Nat.Prime.eq_of_dvd_of_prime pp _ this
    norm_num
  contradiction

theorem primes_mod_4_eq_3_infinite_problem : ∀ n, ∃ p > n, Nat.Prime p ∧ p % 4 = 3 := by
  by_contra h
  push_neg  at h
  rcases h with ⟨n, hn⟩
  have : ∃ s : Finset Nat, ∀ p : ℕ, p.Prime ∧ p % 4 = 3 ↔ p ∈ s := by
...
theorem primes_mod_4_eq_3_infinite_answer : ∀ n, ∃ p > n, Nat.Prime p ∧ p % 4 = 3 := by
  by_contra h
  push_neg  at h
  rcases h with ⟨n, hn⟩
  have : ∃ s : Finset Nat, ∀ p : ℕ, p.Prime ∧ p % 4 = 3 ↔ p ∈ s := by
